...
(defmethod fs-glob ((fs memory-filesystem) pathname pattern &key recursive)
  (let* ((base (fs-absolute fs pathname))
         ...
         (wildcard (if recursive
                       (merge-pathnames (uiop:parse-unix-namestring (format nil "**/~A" pattern))
                                        (uiop:parse-unix-namestring prefix))
                       (uiop:parse-unix-namestring pattern)))
         (out '()))
    (maphash (lambda (k v)
               (declare (ignore v))
               (when (and (uiop:string-prefix-p prefix k)
                          (or recursive (not (find #\/ k :start (length prefix))))
                          (pathname-match-p (if recursive k (file-namestring k)) wildcard))
                 (push (fs-parse fs k) out)))
             (memory-fs-root fs))
    (nreverse out)))
